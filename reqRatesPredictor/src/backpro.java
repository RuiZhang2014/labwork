...
public class backpro {
	...
	public static void main(String args[]){
		...
		double[] pdctres = backpro.dataPrecessing(pdctsrc, "train.in", 12, 12, 12);		//预测结果
		//pdctsrc.add(1.0);
		...
		String res = "[{hour : 0 , rate : "+String.format("%.5f", pdctres[0]) +"}";
		for (int i=1;i<pdctres.length;i++){
			res += ", {hour : " + i + ", rate : " + String.format("%.5f", pdctres[i]) + "}";
		}
		res += "]";
	...
	public static double[] dataPrecessing(ArrayList<Double> src, String filename, int attN, int hidN, int outN){
		//String filename=new String("train.in");
		try {			
			FileInputStream fileInputStream=new FileInputStream(filename);
			Scanner sinScanner=new Scanner(fileInputStream);
			//输入单元个数//隐含层单元个数 //输出单元个数//学习样例个数
